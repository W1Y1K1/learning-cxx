    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        for (int dim = 0; dim < 4; ++dim) {
            ASSERT(others.shape[dim] == 1 || others.shape[dim] == shape[dim],
                   "The right-hand tensor cannot be broadcast to the left-hand tensor");
        }
        ...
        for (unsigned int i0 = 0; i0 < shape[0]; ++i0) {
            for (unsigned int i1 = 0; i1 < shape[1]; ++i1) {
                for (unsigned int i2 = 0; i2 < shape[2]; ++i2) {
                    for (unsigned int i3 = 0; i3 < shape[3]; ++i3, ++index) {
                        const unsigned int other_index =
                            (((i0 % others.shape[0]) * others.shape[1] + i1 % others.shape[1]) *
                                 others.shape[2] +
                             i2 % others.shape[2]) *
                                others.shape[3] +
                            i3 % others.shape[3];
